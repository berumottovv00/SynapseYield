...
class MarketDataTool(BaseTool):
    ...
    parameters = {
        ...
        "properties": {
            ...
            "source": {
                ...
                "description": (
                    "Data source. Leave as 'auto' (recommended) — it detects the right "
                    "free source from the symbol format and falls back automatically. "
                    "Only set this explicitly if 'auto' already failed for these symbols. "
                    "Free, no key: yfinance/yahoo (US/HK equities), okx/ccxt "
                    "(crypto), baostock/tencent/eastmoney/sina/akshare/mootdx "
                    "(China A-shares), stooq (global EOD). Key-gated REST — do NOT pick "
                    "these by default, they require a paid/verified account: tushare "
    ...
    def execute(self, **kwargs: Any) -> str:
        missing = [key for key in ("codes", "start_date", "end_date") if key not in kwargs]
        if missing:
            return json.dumps(
                {
                    "status": "error",
                    "error": f"Missing required parameter(s): {', '.join(missing)}. "
                    "codes, start_date (YYYY-MM-DD), and end_date (YYYY-MM-DD) are all required.",
                },
                ensure_ascii=False,
            )
