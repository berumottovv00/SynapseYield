"""Swarm YAML presets loader.

Reads YAML presets files from the bundled ``presets/`` directory next to this
...
def inspect_preset(name: str) -> dict:
    """Validate a swarm presets and return a dry-run execution plan.
...
def build_run_from_preset(preset_name: str, user_vars: dict[str, str]) -> SwarmRun:
    """Create a SwarmRun from a presets with user variables applied.
    ...
    Args:
        preset_name: Name of the presets to load.
    ...
    Raises:
        ...
        ValueError: If presets YAML is malformed.
    """
