...
# The system prompt asks for a "Phase 1 — Plan (0 tool calls)" turn before any
# tool call. _classify_deliverable's plan-prefix leniency only forgives that
# turn when it's short or ends on a narrow set of handoff phrases (#118); a
# longer-but-legitimate plan otherwise falls straight through to the
# no-tool-calls contract check and kills the task before Phase 2 ever runs.
# Give a plan-only first response a bounded number of nudges back to Phase 2
# instead of failing the task outright the first time the heuristic misses.
_MAX_PLAN_ONLY_NUDGES = 2
...
def run_worker(
    ...
    for iteration in range(max_iterations):
        ...
        # If no tool calls, this looks like a final response — except a
        # plan-only Phase 1 turn (the system prompt explicitly asks for one),
        # which gets a bounded number of nudges back to Phase 2 instead of
        # being judged as the worker's final answer.
        if not response.has_tool_calls:
            content = (response.content or "").strip()
            looks_like_plan_only = content.lower().startswith(_PLAN_PREFIXES)
            if looks_like_plan_only and plan_only_nudge_count < _MAX_PLAN_ONLY_NUDGES:
                plan_only_nudge_count += 1
                _emit(event_callback, "worker_plan_only_nudge", agent_id, task_id,
                      {"iteration": iteration, "count": plan_only_nudge_count})
                messages.append({"role": "assistant", "content": response.content})
                messages.append({
                    "role": "user",
                    "content": (
                        "[SYSTEM] That was Phase 1 (plan only, 0 tool calls) — noted. "
                        "Now move to Phase 2: make actual tool calls (load_skill, "
                        "write_file, bash, etc.) instead of describing them in text."
                    ),
                })
                continue
