...
- `TradingAgentGraph` 使用 LangGraph 管理 Market、Strategy、Risk、Execution 四个 Agent 节点。
...
LangGraph Agent 节点边界：

```text
MarketAgent
  -> 标准化行情为 MarketQuote
StrategyAgent
  -> 调用 StrategyRunner 和 StrategyAdapter
RiskAgent
  -> 调用 OrderService 和 RiskEngine
ExecutionAgent
  -> 只提交已经通过风控的订单给 Broker
```

Agent 只负责流程推进，不直接修改资金、持仓或绕过状态机。
